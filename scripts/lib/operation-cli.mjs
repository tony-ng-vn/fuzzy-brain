...
const safe = async action => { try { return await action(); } catch { return { recorded: false, error_code: "trace_unavailable" }; } };
...
export async function recordTraceInput(value) {
  ...
  if (context?.id) await safe(() => context.journal.recordInput(context.id, parsed(value)));
  return value;
}
...
export async function runTracedCli(entryPoint, operation, args, handler) {
  ...
  const start = await safe(() => journal.start({ entry_point: entryPoint, operation, release, input: args,
  ...
  return operationContext.run(context, async () => {
    ...
    try { result = await handler(); return result; }
      ...
      const finished = start.recorded ? await safe(() => journal.finish(start.id, {
