// The rerank stage: a linear scorer over features retrieval already produced.
//
// Shared by scripts/recall.mjs and experiments/recall-bench/rerank.mjs, which
// re-exports these bound to the bench config. Candidates arrive in fused (RRF)
// order, which matters because dupPenalty counts how many same-group members
// already sit above a candidate.
...
export function rerankFeatures(qf, candidate, cfg = retrievalDefaults) {
  const f = candidate.features ?? {};
  return {
    // The fused RRF score, normalized per query in rerank() below. Without it
    // the rerank REPLACES the retrieval ranking instead of refining it, and
    // the fused order is the strongest single signal either system has.
...
// Ids are dense integers in the bench and uuids in the brain. Numeric ids keep
// the exact `a.id - b.id` ordering the bench was measured under; anything else
// falls back to a string compare, which is deterministic where subtraction
// would produce NaN and leave the sort undefined.
...
export function rerank(qf, candidates, cfg = retrievalDefaults) {
  ...
  // The near-duplicate unblocker: each further member of a dup group already
  // represented above this candidate costs it more. Counted in the incoming
  // fused order, which is the order the reader would have seen.
  ...
  return scored
    .map((s) => s.candidate)
    .sort((a, b) => (b.rerankScore - a.rerankScore) || compareIds(a.id, b.id));
}
